...
pub(crate) fn impl_water_wrap(ast: &syn::DeriveInput) -> TokenStream {
    let name = &ast.ident;
    ...
    #[export_name = "_role"]
    pub fn _role() -> i32 {
        ::water_wasm::common::Role::Wrapper as i32
    }
    ...
    lazy_static! {
        static ref #inst_name: std::sync::Arc<
            std::sync::Mutex<
                std::boxed::Box<dyn ::water_wasm::v0::ReadWriteWrap>>> = {
                    let mut m = std::sync::Arc::new(std::sync::Mutex::new(<#name>::new()));
                    m
                };
    }
